...
exports.CreateProduct = AsyncFunc(async (req, res, next) => {
  let { name, description, price, category, brand, attributes_name, stock } =
    req.body;
  ...
  const uploadImage = (item) => {
    return new Promise(async (resolve, reject) => {
      ...
      const stream = cloudinary.uploader.upload_stream(
        { unique_filename: true, folder: "Shop", chunk_size: 6000000 },
        (error, data) => {
          if (error) {
            reject(error);
          } else {
  ...
  const ImageUploadPromise = Promise.all(
    image_files.map((item) => uploadImage(item))
  );
  ...
  ImageUploadPromise.then((data) => {
    ...
    Product_model.create(
      ...
      (error, data) => {
        ...
        return res
          .status(200)
          .json({ data: "Product has been added successfully." });
      }
    );
  }).catch((error) => {
